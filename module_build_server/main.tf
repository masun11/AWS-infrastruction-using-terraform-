locals {
  ingress_rules = [{
    ...
    port        = 8080
    description = "Ingree rules for tomcat"
    }, {
    port        = 9100
...
resource "aws_instance" "build_server" {
  ...
  private_ip                  = var.shared_data.private_ips["build_server"]
  ...
  user_data = templatefile("${path.module}/init_script.sh.tpl", {
    node_exporter_sh = file("${path.root}/scripts/install-nodeExporter.sh")
    build_server     = file("${path.root}/scripts/setup-build-server.sh")
    install_ansible = templatefile(
      "${path.root}/scripts/install-ansible.sh.tpl", {
        devopsadmin_pass = var.devopsadmin_pass
    })
  })
